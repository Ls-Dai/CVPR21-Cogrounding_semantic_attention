...
from model.cogrounding_model_semantic_attn import *
...
def train_epoch(train_loader, model, optimizer, epoch, size_average):
    ...
    for batch_idx, (imgs, word_id, word_mask, bbox, phrase) in enumerate(train_loader):
        ...
        pred_anchor, sim_score, loc_score, fvisu, flang_attn = model(image, word_id, word_mask)
        ...
        loss = loss_yolo + 100 * loss_rank + loss_loc
        ...
        if batch_idx % args.print_freq == 0:
            print_str = 'Epoch: [{0}][{1}/{2}]\t' \
                ...
                .format( \
                    ...
                    data_time=data_time, loss=losses, yolo_loss=yolo_losses, rank_loss=rank_losses, loc_loss=loc_losses, miou=miou, acc=acc, acc_c=acc_center) 
            print(print_str)
